...
- `dictionary.html`'s definitions are generated by Claude and may occasionally be imprecise — treat it as a writing aid rather than an authoritative reference.
...
- Both files are self-contained single HTML files — no installation or build step required beyond the local server step above.
